...
TEST(Request, testWithParams) {
  ...
  ASSERT_EQ("localhost", lhs::util::any_cast<std::string>(req["Host"]));
  ASSERT_EQ("LHSSample/1.0", lhs::util::any_cast<std::string>(req["User-Agent"]));
  ASSERT_EQ("7", lhs::util::any_cast<std::string>(req["Content-Length"]));
